					...
					u32 cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		dma_addr = (u64)dma_map_single(ctrl->dev,
					       skb->head,
					       SIPA_RECV_BUF_LEN +
					       skb_headroom(skb),
					       DMA_FROM_DEVICE);
		if (dma_mapping_error(ctrl->dev, (dma_addr_t)dma_addr)) {
		...
		node = ctrl->hal_ops.get_rx_fifo_wr(cmn->fifo_id,
						    ...
						    i);
		if (!node) {
			dma_unmap_single(ctrl->dev, dma_addr,
					 SIPA_RECV_BUF_LEN +
					 skb_headroom(skb),
					 DMA_FROM_DEVICE);
			dev_kfree_skb_any(skb);
...
#endif
		if(!ret) {
			dma_unmap_single(ctrl->dev, (dma_addr_t)(addr - ctrl->pcie_mem_offset),
