			      ...
			      enum sipa_cmn_fifo_index id)
{
	...
	cmn_fifo->tx_fifo.virtual_addr = dma_alloc_coherent(ipa->dev, size,
							    &dma_addr,
							    GFP_KERNEL);
	...
	size = cmn_fifo->rx_fifo.depth *
		sizeof(struct sipa_node_description_tag);
	cmn_fifo->rx_fifo.virtual_addr = dma_alloc_coherent(ipa->dev, size,
							    &dma_addr,
							    GFP_KERNEL);
...
int sipa_module_init(struct device  *dev)
{
    ...
    platform_device_register(&sipa_plat_dev);
#endif 
