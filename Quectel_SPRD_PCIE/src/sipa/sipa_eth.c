...
static int sipa_eth_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (dev->type != ARPHRD_ETHER) {
		...
		memcpy(eth_hdr(skb)->h_dest, default_dev_addr, ETH_ALEN);
	}
...
static struct platform_device *sipa_eth_pdev[SIPA_ETH_NUM*2];
static struct sipa_eth_init_data *sipa_eth_data[SIPA_ETH_NUM*2];
...
static int sipa_eth_platform_device_reigster(void) 
{
	...
	for (i = 0; i < 2*SIPA_ETH_NUM; i++) {
		...
		if(-1 < i && i < SIPA_ETH_NUM) {
			snprintf(sipa_eth_data[i]->name, IFNAMSIZ, "%s%d",
			 	SIPA_ETH_IFACE_PREF, i);
                        sipa_eth_data[i]->term_type = 0x6;
                        sipa_eth_data[i]->netid = i;	
                        sipa_eth_data[i]->mac_h = true;		
                }
		if(SIPA_ETH_NUM - 1 < i && i < 2*SIPA_ETH_NUM) {
			snprintf(sipa_eth_data[i]->name, IFNAMSIZ, "%s%d",
			 	SIPA_ETH_VPCIE_PREF, i - SIPA_ETH_NUM);
                        sipa_eth_data[i]->term_type = 0x6;
                        sipa_eth_data[i]->netid = i - SIPA_ETH_NUM;	
                        sipa_eth_data[i]->mac_h = true;			
                 }
